...
const CreatePoint = () => {
    ...
    useEffect(() => {
        ...
        .then(city => {
            const cityNames = city.data.map(city => city.nome)
            setCities(cityNames)
        })
    }, [selectedUf])
    ...
    function handleSelectItem(id: number) {
        const alredySelected = selectId.findIndex(item => item === id)

        if(alredySelected >= 0) {
            const filteredItems = selectId.filter(item=> item !== id)

            setSelectId(filteredItems)
        } else
                    ...
                    <ul className="items-grid" >
                        {items.map(item => (
                             <li 
                                ...
                                className={selectId.includes(item.id) ? 'selected' : ''}
                                >
